...
test("v0.4.0 beta accepts only the paired lifecycle-capable companion", () => {
  const compatibleVersions = ["0.4.0"];
  assert.equal(
    classifyChromeCompanionCompatibility(compatibleVersions, "0.4.0"),
    COMPATIBLE,
  );
  assert.equal(
    classifyChromeCompanionCompatibility(compatibleVersions, "0.3.4"),
  ...
  assert.equal(
    classifyChromeCompanionCompatibility(compatibleVersions, "0.4.1"),
    ZOTERO_UPDATE_REQUIRED,
  );
});
